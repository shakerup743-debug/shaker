 ...
 * Server-side mirror of the same list used in the frontend.
 */
...
export function validateBusinessType(
  ...
  if (!ALLOWED_SLUGS.has(slug)) {
    return {
      ok: false,
      reason: lang === "ar" ? "نوع النشاط غير صالح." : "Invalid business type.",
    };
  }
  ...
  if (text.length > 80) {
    return {
      ok: false,
      reason: lang === "ar" ? "نوع النشاط طويل جداً." : "Business type too long.",
    };
  }
...
/** Return human label for a slug (or the raw custom text when slug === "other"). */
export function businessTypeLabel(
  slug: string,
  customText: string | undefined,
  lang: "ar" | "en" = "ar",
): string {
  if (slug === "other") return (customText ?? "").trim();
  const found = RESTAURANT_BUSINESS_TYPES.find((b) => b.slug === slug);
