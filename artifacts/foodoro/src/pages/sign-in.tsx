  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Clean the hash so reload doesn't re-trigger this
        window.history.replaceState({}, "", window.location.pathname);
        setGoogleLoading(false);
        setWelcomeUser(data.user);
        // Smooth transition: show welcome for 1.5s, then redirect to dashboard
        setTimeout(() => {
  ...
  if (welcomeUser) {
    return (
      ...
      >
        ...
        <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-[#E67E22]/10 rounded-full blur-3xl pointer-events-none" />

        <div className="relative z-10 flex flex-col items-center gap-5 text-center animate-in fade-in zoom-in duration-500">
          <div className="w-20 h-20 rounded-full bg-emerald-500/20 border-2 border-emerald-500 flex items-center justify-center">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-emerald-400">
              <polyline points="20 6 9 17 4 12" />
            </svg>
          </div>
          <div>
            <h2 className="text-white text-2xl font-bold">
              {isAr ? `أهلاً ${welcomeUser.name}!` : `Welcome, ${welcomeUser.name}!`}
            </h2>
            <p className="text-gray-400 text-sm mt-2">
              {isAr ? "تم تسجيل دخولك، نُحضّر لك لوحة التحكم..." : "You're signed in. Loading your dashboard..."}
            </p>
          </div>
          <div className="w-8 h-8 border-2 border-[#E67E22] border-t-transparent rounded-full animate-spin mt-2" />
        </div>
      </div>
    );
  }
        ...
        <div className="relative z-10 flex flex-col items-center gap-6 max-w-sm text-center">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs mt-2">
              {isAr
                ? "اختر حساب Google، نحفظ بياناتك في نظامك ونعيدك للوحة التحكم"
                : "Choose your Google account — we save your data in your system and bring you back"}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="w-full bg-[#1F2937]/80 backdrop-blur-xl rounded-2xl p-6 flex flex-col gap-4 border border-white/5">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              {isAr ? "اختر طريقة الدخول المفضلة لديك" : "Choose your preferred sign-in method"}
            </p>
          </div>
          ...
          <p className="text-[10px] text-gray-500 text-center -mt-2">
            {isAr
              ? "🔒 ضغطة واحدة — يُنشئ مطعمك تلقائياً ويسجل دخولك"
              : "🔒 One click — auto-creates your restaurant and signs you in"}
          </p>
